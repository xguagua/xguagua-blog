...
在这里我们列举几个狄利克雷卷积的常用函数：
- 常数函数 $\mathbf{1}(n) = 1$
- 幂函数 $Id _ k (n) = n ^ k$，特别的，$k = 1$ 时记作 $Id(n)$
- 除数函数 $\sigma _ k(n) = \sum \limits _ {d \mid n} d ^ k$，在 $k = 1$ 时为约数和函数，记作 $\sigma (n)$，$k = 0$ 时为约数个数函数，记为 $d(n)$
- 单位函数 $\epsilon(n) = [n = 1]$，相当于狄利克雷卷积的单位元，满足 $\forall f,f \ast \epsilon = f$
- 莫比乌斯函数 $\mu (n)$
- 欧拉函数 $\varphi (n)$
