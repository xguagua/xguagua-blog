...
struct Chtholly
{
    ...
    Chtholly(int l,int r,int val):l(l),r(r),val(val){};
};
set<Chtholly> odt;
...
具体证明看 CF 上 ODT(lxl) 的证明，大致意思是段数是 $\Omicron(\log n)$ 级别的，然后对颜色段的操作复杂度是 $\Omicron(\log n)$ 的，所以总复杂度是 $\Omicron(n \log \log n)$ 的。

总的来说，用 `set` 实现的是 $\Omicron(n \log \log n)$，`map` 和链表都是 $\Omicron(n \log n)$。

## 3.扩展 ODT

由上得，ODT 的命根子是区间推平，可是有的毒瘤出题人，用构造的全是查询的数据来卡掉 ODT。

这个时候，就该用上扩展 ODT 了。

知周所众，替罪羊树有不用旋转还能平衡的秘诀就是，只要不满意，就炸了重构...
...
还有一种用法：节点对区间的映射，但是模板都 ~~nmd~~ 是黑题，不讲了。
