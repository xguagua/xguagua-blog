...
我们可以发现，$f(x)$ 的所有导数也满足函数关系，所以我们可以把 $f(x)$ 的所有导数组成一个函数，叫做 $f(x)$ 的导函数，简称 **导数** （注意和上面的导数区分，这个是个函数，上面的是个数），记作 $\frac{df(x)}{dx},f'(x),\frac{d}{dx} f(x)$

类似的，我们可以再对导函数求导，称为 $f(x)$ 的二阶导，记作 $f''(x),\frac{d ^ 2}{dx ^ 2}f(x)$，然后满足定义就可以再导，记为 $f ^ {(n)}(x),\frac{d ^ n}{dx ^ n} f(x)$
