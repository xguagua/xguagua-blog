...
具体来讲，假设一个节点维护的是 $[l,r]$ 的信息并，那我们额外维护 $[l,mid]$ 的后缀和与 $[mid + 1,r]$ 的前缀和。

然后，对于一个区间 $[L,R]$ 的查询，我们可以找到节点 $[L,L]$ 与节点 $[R,R]$ 的 LCA，设其为 $P$。显然，$[L,R] \subset P$，我们只需要将 $P$ 中 $[L,mid]$ 与 $[mid + 1,R]$ 的信息合并起来就行。

我们~~惊奇的~~发现，如果用堆式建树，那么 LCA 的编号就是两个叶子节点编号的 LCP，即 ```x >> bits[x ^ y]```，其中 ```x,y``` 是两节点编号，```bits[x]``` 表示 $\lfloor \log _ 2 x \rfloor + 1$。
...
我们发现，这和上面写的猫树的一次查询很像，于是我们考虑将一层视作一个节点，求 $[l,mid]$ 的后缀背包与 $[mid + 1,r]$ 的前缀背包，然后进行合并。
