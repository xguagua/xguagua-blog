...
设 $a,b$ 为不全为 $0$ 的整数，则 $\forall x,y \in \Z,\gcd(a,b) \mid ax + by$，并且 $\exist x,y \in Z,ax + by = \gcd(a,b)$。
...
求解的是如下一元线性同余方程组：$\begin{cases} x \equiv a _ 1 \pmod {n _ 1} \\ x \equiv a _ 2 \pmod {{n _ 2}} \\ \vdots \\ x \equiv a _ k \pmod {n _ k}\end{cases}$，条件是所有 $n _ i$ 两两互质。

做法如下：

1. 求出 $N = \prod n _ i$
2. 对于第 $i$ 个方程，求出 $m _ i = \frac{N}{n _ i}$ 与 $m _ i$ 在模 $n _ i$ 意义下的逆元 $m _ i ^ {-1}$，最后答案为 $\sum a _ i m _ i m _ i ^ {-1} \pmod N$。
...
设有两个方程 $x \equiv a _ 1 \pmod {n _ 1},x \equiv a _ 2 \pmod {n _ 2}$。

则可以转化为不定方程 $x = k _ 1 n _ 1 + a _ 1,x = k _ 2 n _ 2 + a _ 2$，则有 $k _ 1 n _ 1 - k _ 2 n _ 2 = a _ 2 - a _ 1$。

由 Bezout 定理，当 $\gcd(n _ 1,n _ 2) \nmid a _ 2 - a _ 1$ 时无解。

否则可以用 exGCD 求出来一组可行的 $k _ 1,k _ 2$，两原方程可以合并为 $x \equiv k _ 1 n _ 1 + a _ 1 \pmod {\operatorname{lcm}(n _ 1,n _ 2)}$。
